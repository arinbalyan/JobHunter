...
### 1. Scrape stores no per-site job counts
...
### 3. Dashboard needs Vercel env vars
...
### 4. Telegram report missing for send workflow
...
### 5. config.toml has no validation
...
### 8. Onsite vs remote email templates
...
### 9. Release notes include old copilot PRs
...
- **scrappy v0.3.10**: ScrapeResult with per-site stats (item 3) — not yet released. Would fix item 1 above.
